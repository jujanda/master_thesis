...
%developed by \cite{LAKM+16} \\
%aims to replicate performance of \textbf{Differential Mean Opinion Score (DMOS)} from subjective testing \\
%Single metrics on their own not sufficient: each fall flat in different scenarios \\
%Uses three metrics: \\
%Visual Information Fidelity (VIF) \cite{ShBo06} \\
%Detail Loss Metric (DLM) \cite{LZMN11} \\
%Motion: \\
%Combines all three via machine learning using support vector machine (SVM) regression \cite{CoVa95} \\
%results in score ranging from 0 to 100, with higher being better.
%
